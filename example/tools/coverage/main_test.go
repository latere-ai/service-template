...
func TestRunPassesAndFailsOnTheSameProfileSet(t *testing.T) {
	...
	profile := writeProfile(t, dir, "cover.out",
		"mode: atomic\n"+
			"github.com/example/reference-service/internal/a/f.go:1.1,2.2 9 1\n"+
			"github.com/example/reference-service/internal/a/f.go:3.1,4.2 1 0\n")

	pass := "module: github.com/example/reference-service\ncoverage:\n  threshold: 90\n"
	...
	fail := "module: github.com/example/reference-service\ncoverage:\n  threshold: 95\n"
...
func TestRunMergesEveryProfileFlag(t *testing.T) {
	dir := t.TempDir()
	config := filepath.Join(dir, ".template.yaml")
	if err := os.WriteFile(config, []byte("module: github.com/example/reference-service\ncoverage:\n  threshold: 100\n"), 0o644); err != nil {
		t.Fatalf("write declaration: %v", err)
	}
	unit := writeProfile(t, dir, "unit.out",
		"mode: atomic\ngithub.com/example/reference-service/internal/a/f.go:1.1,2.2 5 1\n"+
			"github.com/example/reference-service/internal/a/f.go:3.1,4.2 5 0\n")
	integration := writeProfile(t, dir, "integration.out",
		"mode: atomic\ngithub.com/example/reference-service/internal/a/f.go:1.1,2.2 5 0\n"+
			"github.com/example/reference-service/internal/a/f.go:3.1,4.2 5 1\n")
...
func TestRunReportsAMissingDeclaration(t *testing.T) {
	dir := t.TempDir()
	profile := writeProfile(t, dir, "cover.out",
		"mode: atomic\ngithub.com/example/reference-service/internal/a/f.go:1.1,2.2 10 1\n")
