...
func TestParseMergesTiers(t *testing.T) {
	...
	unit := "mode: atomic\n" +
		"example.com/service/internal/a/f.go:1.1,2.2 3 1\n" +
		"example.com/service/internal/a/f.go:3.1,4.2 2 0\n"
	integration := "mode: atomic\n" +
		"example.com/service/internal/a/f.go:1.1,2.2 3 0\n" +
		"example.com/service/internal/a/f.go:3.1,4.2 2 5\n"

	pkgs := load(t, unit, integration).Packages("example.com/service")
	if len(pkgs) != 1 {
...
func TestParseRejectsMalformedLines(t *testing.T) {
	for name, text := range map[string]string{
		"too few fields":  "mode: set\nexample.com/service/a/f.go:1.1,2.2 3\n",
		"no span":         "mode: set\nexample.com/service/a/f.go 3 1\n",
		"bad statements":  "mode: set\nexample.com/service/a/f.go:1.1,2.2 x 1\n",
		"bad occurrences": "mode: set\nexample.com/service/a/f.go:1.1,2.2 3 y\n",
	} {
...
func TestThresholdBothDirections(t *testing.T) {
	...
	text := profileText("example.com/service", map[string][][2]int{
		"internal/a": {{9, 1}, {1, 0}},
	})
	set := load(t, text)
...
func TestPackageFarBelowThresholdFailsAlone(t *testing.T) {
	...
	atFloor := profileText("example.com/service", map[string][][2]int{
		"internal/big":   {{180, 1}, {2, 0}},
...
func TestPackageWithNoTestsIsReportedMissing(t *testing.T) {
	...
	known := []string{"internal/a", "internal/b", "cmd/service"}
	...
	if strings.Contains(problems[0], "cmd/service") {
...
func TestRelativeTo(t *testing.T) {
	cases := map[string]string{
		"example.com/service":            ".",
		"example.com/service/internal/a": "internal/a",
		"other.example/pkg":              "other.example/pkg",
	}
	for in, want := range cases {
		if got := relativeTo("example.com/service", in); got != want {
...
func TestReadDeclaration(t *testing.T) {
	...
	body := "module: example.com/service\ncoverage:\n  threshold: 85\n  exclude:\n    - cmd/\n"
...
func TestWriteReportsEveryPackageAndTheOverall(t *testing.T) {
	...
	out := b.String()
	for _, want := range []string{"internal/a", "cmd/service", "excluded", "overall", "threshold 90%", "90.0%"} {
